...
const emit = defineEmits<{
  ...
  'update:session': [session: AiSession]
}>()
// 当鼠标放到会话上时，会弹出删除图标，点击删除图标调用删除接口并发送删除事件。
const handleDeleteSession = () => {
  api.aiSessionForFrontController.delete({ body: [prop.session.id] }).then(() => {
    emit('delete', prop.session)
  })
}
const onChooseChange = (val: any, session: AiSession) => {
  emit('update:session', { ...session, checked: val })
}
</script>
...
<template>
  <div class="session-wrapper">
    <el-checkbox
      class="check-box"
      :model-value="session.checked"
      @change="onChooseChange($event, session)"
    ></el-checkbox>
