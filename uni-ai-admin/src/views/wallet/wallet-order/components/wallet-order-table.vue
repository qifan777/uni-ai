...
type WalletOrderScope = Scope<WalletOrderDto['WalletOrderRepository/COMPLEX_FETCHER_FOR_ADMIN']>
...
const handleCreate = () => {
  openDialog('CREATE')
  createForm.value = { ...walletOrderStore.initForm }
}
...
const handleDelete = (ids: string[]) => {
  ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
    ...
    api.walletOrderForAdminController.delete({ body: ids }).then((res) => {
  ...
  <div>
    ...
    >
      ...
      <el-table-column
        ...
        width="120"
      >
        <template v-slot:default="{ row }: WalletOrderScope">
          {{ row.amount }}
        </template>
      </el-table-column>
      <el-table-column
