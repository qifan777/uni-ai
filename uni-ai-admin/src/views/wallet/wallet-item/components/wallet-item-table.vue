...
import type { WalletItemDto } from '@/apis/__generated/model/dto'
import { Delete, Edit, Plus } from '@element-plus/icons-vue'
import DictColumn from '@/components/dict/dict-column.vue'
import { DictConstants } from '@/apis/__generated/model/enums/DictConstants'
type WalletItemScope = Scope<WalletItemDto['WalletItemRepository/COMPLEX_FETCHER_FOR_ADMIN']>
...
const handleCreate = () => {
  openDialog('CREATE')
  createForm.value = { ...walletItemStore.initForm }
}
...
const handleDelete = (ids: string[]) => {
  ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
    ...
    api.walletItemForAdminController.delete({ body: ids }).then((res) => {
  ...
  <div>
    ...
    >
      ...
      <el-table-column
        ...
        width="120"
      >
        <template v-slot:default="{ row }: WalletItemScope">
          {{ row.name }}
        </template>
      </el-table-column>
      <el-table-column
