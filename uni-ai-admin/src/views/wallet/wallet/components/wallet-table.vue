...
type WalletScope = Scope<WalletDto['WalletRepository/COMPLEX_FETCHER_FOR_ADMIN']>
...
const { pageData, loading, queryRequest, table, updateForm, createForm } = storeToRefs(walletStore)
onMounted(() => {
  reloadTableData()
})
...
const handleCreate = () => {
  openDialog('CREATE')
  createForm.value = { ...walletStore.initForm }
}
...
const handleDelete = (ids: string[]) => {
  ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
    ...
    api.walletForAdminController.delete({ body: ids }).then((res) => {
  ...
  <div>
    ...
    >
      ...
      <el-table-column
        ...
        width="120"
      >
        <template v-slot:default="{ row }: WalletScope">
          {{ row.balance }}
        </template>
      </el-table-column>
      <el-table-column
